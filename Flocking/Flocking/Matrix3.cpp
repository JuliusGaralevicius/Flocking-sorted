...
Vector3 Matrix3::ToEuler() const {
	...
	if (testVal < 1.0f) {
		...
		float psi1 = RadiansToDegrees(atan2(values[5] / cost1, values[8] / cost1));
		float psi2 = RadiansToDegrees(atan2(values[5] / cost2, values[8] / cost2));

		float phi1 = RadiansToDegrees(atan2(values[1] / cost1, values[0] / cost1));
		float phi2 = RadiansToDegrees(atan2(values[1] / cost2, values[0] / cost2));
